...
# 0.2 Get the detection
if [ -z "$2" ]; then
    echo "Error: Please provide a detection name (delta_dagmm, fgfl, etc.)"
    exit 1
fi
DETECTION="$2"
    ...
    for NUM_C in "${NUM_CLIENTS[@]}"; do
        ...
        for PERC_M in "${PERCENT_MALICIOUS[@]}"; do
            ...
            # Run everything 3 times
                ...
                python generate_config.py "$DATASET" $NUM_C $PERC_M "$ATTACK" "$DETECTION"
...
echo "Finished the experiment for $DATASET with $DETECTION"