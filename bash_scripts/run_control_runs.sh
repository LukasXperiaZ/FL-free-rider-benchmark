...
DETECTIONS_STRING=$(python get_detection_names.py)
readarray -t DETECTIONS <<< "$DETECTIONS_STRING"
...
TOTAL_EXPERIMENTS=$((${#RUN_3_TIMES[@]} *  ${#DETECTIONS[@]} * ${#NUM_CLIENTS[@]} * ${#PERCENT_MALICIOUS[@]} ))
    ...
    for NUM_C in "${NUM_CLIENTS[@]}"; do
        ...
        for PERC_M in "${PERCENT_MALICIOUS[@]}"; do
            ...
            # Run everything 3 times
                ...
                python generate_config.py "$DATASET" $NUM_C $PERC_M "no_attack" "$DETECTION"
